...
const { getPuzzleSessionMock, evaluatePuzzleAttemptMock, recordPuzzleSessionMock } =
  vi.hoisted(() => {
  return {
    getPuzzleSessionMock: vi.fn(),
    evaluatePuzzleAttemptMock: vi.fn(),
    recordPuzzleSessionMock: vi.fn(),
  };
});
...
vi.mock('../../lib/progress', () => {
  return {
    recordPuzzleSession: recordPuzzleSessionMock,
  };
});
...
describe('PuzzlePage', () => {
  beforeEach(() => {
    ...
    recordPuzzleSessionMock.mockReset();
  });
  ...
  it('persists session summary after completion', async () => {
    const user = userEvent.setup();
    ...
    recordPuzzleSessionMock.mockResolvedValue({
    ...
    await user.click(screen.getByRole('button', { name: /Case h2, pièce pion noir/i }));
    await user.click(screen.getByRole('button', { name: /Case h1/i }));
    ...
    await waitFor(() => {
      expect(recordPuzzleSessionMock).toHaveBeenCalledWith({
        accessToken: 'access-token-1',
        totalPuzzles: 1,
        solvedPuzzles: 1,
        skippedPuzzles: 0,
